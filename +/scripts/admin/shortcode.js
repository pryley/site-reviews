...
const Shortcode = function (selector) {
    ...
    this.create = function (editor_id) {
        ...
        (new Ajax(request)).post(this.handleResponse_.bind(this));
    };
...
Shortcode.prototype = {
    ...
    close_: function (el, button) {
        ...
        jQuery(el).find('.glsr-mce-menu').hide();
    },
    ...
    normalizeDisplay_: function (key) {
        if ('display' !== key || jQuery.isNumeric(this.attributes_[key])) return;
    ...
    onClose_: function (el, button, ev) {
        if (jQuery(ev.target).closest(jQuery(el)).length) return;
    ...
    open_: function (el, button) {
        ...
        jQuery(el).find('.glsr-mce-menu').show();
    },
