...
/*
 * Per-test isolation, applied to every suite through its Pest.php: the four
 * main suites (tests/pest/Pest.php) and the WooCommerce suite
 * (tests/woocommerce/Pest.php). Three kinds of state leak between tests and
 * are reset here:
 *
 ...
 *
 * resetPluginState() (helpers.php) restores a known-good baseline of plugin
 * settings from a beforeEach in the files that need one — not globally, since
 * replacing the whole options array is not free and most tests skip it.
 *
 * The closures are created here, at file scope, on purpose: Pest binds a
 * beforeEach/afterEach closure to the test case, and a closure created inside
 * a static method is static, which cannot be bound.
 */
