...
install_plugins() {
    ...
    download https://downloads.wordpress.org/plugin/elementor.zip $TMPDIR/elementor.zip
    unzip -d $WP_CORE_DIR/wp-content/plugins/ $TMPDIR/elementor.zip
    # we need GamiPress for the code coverage
    download https://downloads.wordpress.org/plugin/gamipress.zip $TMPDIR/gamipress.zip
