<?php defined('ABSPATH') || exit; ?>

<p class="about-description">We hope you love the changes in this new release!</p>
<div class="is-fullwidth">
    <div class="glsr-flex-row">
        <div class="glsr-column">
            <?php include trailingslashit(__DIR__).'whatsnew/v65.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v64.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v62.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v61.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v60.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v525.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v524.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v523.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v522.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v521.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v520.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v519.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v518.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v517.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v516.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v515.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v514.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v513.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v512.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v511.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v510.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v59.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v58.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v57.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v56.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v55.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v54.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v53.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v52.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v51.php'; ?>
            <?php include trailingslashit(__DIR__).'whatsnew/v50.php'; ?>
        </div>
    </div>
</div>
