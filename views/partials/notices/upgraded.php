...
<div class="notice is-dismissible glsr-notice" data-dismiss="upgraded">
    ...
    <p class="glsr-notice-buttons">
        <?php if (glsr()->hasPermission('welcome', 'whatsnew')) { ?>
            <a class="button" href="<?php echo esc_url(glsr_admin_url('welcome', 'whatsnew')); ?>"><?php echo _x("See What's New", 'admin-text', 'site-reviews'); ?></a>
        <?php } ?>
        <?php if (glsr()->hasPermission('welcome', 'upgrade-guide')) { ?>
            <a class="button button-link" href="<?php echo esc_url(glsr_admin_url('welcome', 'upgrade-guide')); ?>"><?php echo _x('Read the Upgrade Guide', 'admin-text', 'site-reviews'); ?> →</a>
        <?php } ?>
    </p>
</div>
