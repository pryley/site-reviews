...
<div class="form-field term-<?php echo esc_attr($id); ?>-wrap">
    ...
    <p class="description" id="<?php echo esc_attr($id); ?>-description">
        <?php /* translators: %s: the {{ assigned_terms }} template tag */ printf(_x('Categories with a higher priority will be displayed first when using the %s template tag in the review.', '{{ assigned_terms }} (admin-text)', 'site-reviews'), '<code style="white-space:nowrap;">{{ assigned_terms }}</code>'); ?>
    </p>
</div>
