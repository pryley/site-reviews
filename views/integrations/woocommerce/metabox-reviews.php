...
<div style="padding: 0 12px;">
    <p style="margin: 1em 0;" class="inline notice notice-info">
        <?= _x('Your product reviews are being managed by Site Reviews.', 'admin-text', 'site-reviews'); ?>
    </p>
    <p style="padding: 0; margin: 1em 0;">
        <?php if (0 === $ratings->reviews) { ?>
            <?= _x('This product has no reviews.', 'admin-text', 'site-reviews'); ?>
        <?php } else { ?>
            <a href="<?= add_query_arg('assigned_post', $postId, glsr_admin_url()); ?>" class="button">
                <?php printf(
