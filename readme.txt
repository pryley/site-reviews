...
- Added support for WooCommerce's "Customer review request" emails — submitted reviews are saved in Site Reviews, and customers are not asked to review a product twice
- Fixed a "doing it wrong" notice raised by an integration's version notice
- Fixed a duplicate Reviews section on product pages using WooCommerce's redesigned Product Details block
...
- Fixed the approve link in the notification email using a different capability than the Approve action on the reviews screen
...
- Fixed the notification and verification email settings offering template tags that were never replaced in those emails
